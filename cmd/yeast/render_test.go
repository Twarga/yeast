...
func TestRenderCommandOutputJSONForCoreCommands(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			requiredKeys: []string{"project_root", "project_id", "config_path", "metadata_path", "template", "created"},
		},
		{
			name:    "template-list",
			command: "init",
		...
		},
		{
			...
			requiredKeys: []string{"image_name", "image_path", "manifest_url", "sha256"},
		},
		{
			name:    "doctor",
			command: "doctor",
		...
		},
		{
			...
			requiredKeys: []string{"project_id", "instance", "snapshot", "snapshot.name", "snapshot.source_disk_size"},
		},
		{
			name:    "restore",
			command: "restore",
